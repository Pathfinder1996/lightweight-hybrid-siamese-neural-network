...
def inverted_residual_block_se_pre(x, expand_ratio, out_channels, stride=1, reduction=16):
    in_channels = x.shape[-1]
    shortcut = x
...
def inverted_residual_block_se_post(x, expand_ratio, out_channels, stride=1, reduction=16):
    in_channels = x.shape[-1]
    shortcut = x
...
def inverted_residual_block_se_identity(x, expand_ratio, out_channels, stride=1, reduction=16):
    in_channels = x.shape[-1]
    # Apply SE to identity branch
    if stride == 1 and in_channels == out_channels:
        shortcut = se_block(x, reduction=reduction)
    else:
        shortcut = x
