...
def compute_eer(fpr, tpr, thresholds):
    ...
    # Linear interpolation for more accurate EER
    if min_index == 0 or min_index == len(fpr) - 1:
        eer = (fpr[min_index] + fnr[min_index]) / 2
        eer_threshold = thresholds[min_index]
    else:
        x0, x1 = fpr[min_index - 1], fpr[min_index + 1]
        y0, y1 = fnr[min_index - 1], fnr[min_index + 1]
        eer = (x0 + y0 + x1 + y1) / 4
        eer_threshold = thresholds[min_index]
