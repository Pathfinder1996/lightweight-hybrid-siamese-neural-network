...
for network_name in network_names:
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(pairs_1, labels)):
        ...
        trainPairs_0 = train_pairs_1.reshape(-1, 64, 64, 1)
        trainPairs_1 = train_pairs_2.reshape(-1, 64, 64, 1)
        valPairs_0   = val_pairs_1.reshape(-1, 64, 64, 1)
        valPairs_1   = val_pairs_2.reshape(-1, 64, 64, 1)
        ...
        model_checkpoint = ModelCheckpoint(
            os.path.join(result_dir, f"{network_name}_model_fold_{fold + 1}.keras"), 
            monitor="val_loss", 
            verbose=1, 
            save_best_only=True
        )
        ...
        best_ckpt_path = os.path.join(result_dir, f"{network_name}_model_fold_{fold + 1}.keras")
        ...
        for dataset_name in dataset_list:
            ...
            # Test data directory
            test_image_dir1 = fr"E:\TEST\new_test\test\{dataset_name}\s1"
            test_image_dir2 = fr"E:\TEST\new_test\test\{dataset_name}\s2"
            ...
            testPairs_0 = test_pairs_1.reshape(-1, 64, 64, 1)
            testPairs_1r= test_pairs_2.reshape(-1, 64, 64, 1)
            ...
            if len(thresholds) < 1000:
                custom_thresholds = np.linspace(np.min(y_pred), np.max(y_pred), 1000)
                ...
                fpr = np.array(fpr_custom)
                tpr = np.array(tpr_custom)
