...
from keras.applications import ( 
    ...
    EfficientNetB0, EfficientNetB1, ResNet50, VGG16, VGG19
)
...
from blocks import (
    ...
    se_block, h_swish, h_sigmoid, inverted_residual_block_se_identity,
    inverted_residual_block_se_pre, inverted_residual_block_se_post
)
...
def ResNet34(input_shape):
    ...
    model = Model(inputs=inputs, outputs=x, name="ResNet34")
    model.summary()
...
def VGG_19(input_shape):
    base_model = VGG19(weights=None, input_shape=input_shape, include_top=False)
...
# MobileNetV3Small
def MobileNetV3_Small(input_shape):
    base_model = MobileNetV3Small(weights=None, input_shape=input_shape, include_top=False)
    
    x = GlobalAveragePooling2D()(base_model.output)
    x = Dense(128)(x)

    model = Model(inputs=base_model.input, outputs=x, name="MobileNetV3_Small")
    model.summary()
    return model

def MobileNetV3_Large(input_shape):
    base_model = MobileNetV3Large(weights=None, input_shape=input_shape, include_top=False)

    x = GlobalAveragePooling2D()(base_model.output)
    x = Dense(128)(x)

    model = Model(inputs=base_model.input, outputs=x, name="MobileNetV3_Large")
    model.summary()
...
# EfficientNetB1
def EfficientNet1(input_shape):
    base_model = EfficientNetB1(weights=None, input_shape=input_shape, include_top=False)

    x = GlobalAveragePooling2D()(base_model.output)
    x = Dense(128)(x)

    model = Model(inputs=base_model.input, outputs=x, name="EfficientNetB1")
    model.summary()
...
def Ours(input_shape):
    ...
    for i in range(4):
        if i == 0:
            x = inverted_residual_block_se(x, expand_ratio=1, out_channels=filters[i], stride=strides[i])
        else:
            x = inverted_residual_block_se(x, expand_ratio=6, out_channels=filters[i], stride=strides[i])
...
def Ours_se_pre_block(input_shape):
    inputs = Input(shape=input_shape)
    ...
    for i in range(4):
        if i == 0:
            x = inverted_residual_block_se_pre(x, expand_ratio=1, out_channels=filters[i], stride=strides[i])
        else:
            x = inverted_residual_block_se_pre(x, expand_ratio=6, out_channels=filters[i], stride=strides[i])
...
def Ours_se_post_block(input_shape):
    inputs = Input(shape=input_shape)
    ...
    for i in range(4):
        if i == 0:
            x = inverted_residual_block_se_post(x, expand_ratio=1, out_channels=filters[i], stride=strides[i])
        else:
            x = inverted_residual_block_se_post(x, expand_ratio=6, out_channels=filters[i], stride=strides[i])
...
def Ours_se_identity_block(input_shape):
    inputs = Input(shape=input_shape)
    ...
    for i in range(4):
        if i == 0:
            x = inverted_residual_block_se_identity(x, expand_ratio=1, out_channels=filters[i], stride=strides[i])
        else:
            x = inverted_residual_block_se_identity(x, expand_ratio=6, out_channels=filters[i], stride=strides[i])
...
def get_network(name, input_shape):
    networks = {
        ...
        "VGG16": VGG_16,
        "VGG19": VGG_19,
        "Without SE Block": Ours_without_se_block,
        "SE PRE Block": Ours_se_pre_block,
        "SE POST Block": Ours_se_post_block,
        "SE Identity Block": Ours_se_identity_block,
    }
