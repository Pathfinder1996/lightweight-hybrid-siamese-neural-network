...
for dataset_name in dataset_list:
    ...
    for res in comparison_results:
        ...
        # Plot DET curve directly from original points (no interpolation)
        plt.plot(fpr_, fnr_, label=f"{model_name} (EER={eer_:.2%})")

        # Find closest point to EER for marker (no interpolation)
        diff_array = np.abs(fpr_ - fnr_)
        min_index = np.argmin(diff_array)
        eer_x = fpr_[min_index]
        eer_y = fnr_[min_index]
        plt.scatter(eer_x, eer_y, s=15, color="black", zorder=10)
